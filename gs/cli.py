...
from gs import __app_name__, __version__, __desc__, config, database, ERRORS, gs
...
def get_todoer() -> gs.Todoer:
    if config.CONFIG_FILE_PATH.exists():
        db_path = database.get_database_path(config.CONFIG_FILE_PATH)
    else:
        typer.secho(
            'Config file not found. Please, run "gs init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
    if db_path.exists():
        return gs.Todoer(db_path)
    else:
        typer.secho(
            'Database not found. Please, run "gs init"',
            fg=typer.colors.RED,
        )
        raise typer.Exit(1)
